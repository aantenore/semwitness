...
| Passport-as-credential confusion  | A content-free lineage Statement is treated as permission to serve a cache hit                                        | Predicate fixes `authentication: none`, `decision: shadow-qualified`, and `activationCeiling: shadow-only`; API reports only `bound`; no serving adapter                                                                         | An external consumer can still ignore the contract; active delivery requires a separate authenticated approval and active-admission protocol                                                                                              |
| Decision-as-credential confusion  | An eligible per-hit Decision Statement is treated as current authorization or permission to return its value          | Predicate fixes `authentication: none`, `mode: shadow`, `applied: false`, `activationCeiling: shadow-only`, and `servingAuthority: none`; no serving adapter                                                                     | A modified consumer can ignore the ceiling; a future active protocol needs authenticated approval, current authorization, clock/revocation, and replay state                                                                              |
| Passport/witness substitution     | A valid Passport from one qualification is paired with a hit, operation, dependency set, entry, or value from another | Two exact-payload subjects; Passport binding; cache-hit integrity; exact namespace/tenant, operation, ontology, normalizer, policy, entry/value and plan registry/planner/tool cross-links; recomputed cache key and commitments | The witness does not bind the qualification deployment-scope digest or full provider/model/store inventory; those Statement fields are qualification-declared, not per-hit proof; unsigned source evidence is not producer authentication |
| Admission value dictionary attack | Public entry or value SHA reveals a low-entropy plan/value, or stable metadata reveals repeated work                  | Predicate omits public entry/value SHA and uses binding-scoped, domain-separated HMAC commitments; raw source/value/identifiers are absent                                                                                       | Commitments, source HMAC, cache key, subjects, and lineage digests remain linkable under repeated deployment keys and inputs                                                                                                              |
| Admission private-input leakage   | Candidate value or deployment HMAC secret appears in argv, stdout, errors, logs, unsafe files, or symlink targets     | CLI accepts only a `SEMWITNESS_*` environment-variable name, reads bounded regular files, emits content-free results, rejects clobber/symlinks, and writes `0600` on POSIX                                                       | Windows requires an owner-restricted parent-directory DACL; process environment, shell history, crash tooling, and external tracing remain operator responsibilities                                                                      |
| Admission Decision replay         | An old unsigned Decision Statement is presented after authorization, validity, revocation, freshness, or keys change  | v0.1 has no serving authority and explicitly claims no clock/revocation/current-authorization/replay enforcement; exact-byte binding does not become freshness                                                                   | The same Statement can be replayed as historical evidence; any future active profile requires a new authenticated protocol and atomic replay store                                                                                        |
| Transparency-receipt confusion    | SCITT/COSE registration evidence is treated as proof that the admission decision is correct or safe to serve          | The artifact is named Decision Statement; transparency receipts remain separate and can prove only registration/inclusion of exact bytes                                                                                         | External governance can still over-trust log inclusion or a compromised transparency service                                                                                                                                              |
...
| Passport replay                   | An old Statement is replayed after policy, deployment, validity, or revocation changes                                | Predicate v0.1 has no serving authority; copied RFC 3339 validity/revocation claims are explicit but not enforced; a future active protocol requires expiry, nonce/sequence, and replay state                                    | Equality and workload shape remain visible to anyone who can read the Statement                                                                                                                                                           |
