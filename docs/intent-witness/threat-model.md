...
| Credential/config leakage         | API keys are committed in JSON, printed in errors, or injected through config                                    | Strict versioned binding; only `apiKeyEnv` matching `SEMWITNESS_*`; unknown fields rejected; content-free errors; no raw provider diagnostics                                 | Process environment and external provider tracing remain host/operator responsibilities                                                                        |
