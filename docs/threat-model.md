...
- Promotion Evidence Workbench inputs are likewise host attestations. The
  workbench can prove strict structure, completeness, deterministic arithmetic,
  and digest binding, but it cannot prove that a corpus was genuinely held out
  or that a provider/runtime/task oracle reported honestly.
...
15. A generated promotion requires at least 50 complete paired held-out cases,
    randomized or counterbalanced order, all four difficulty strata, cold/warm
    execution, at least five cases per stratum/cache cell, at least ten complete
    cases per codec, unique case/trace/quality digests, exact observed usage,
    zero unsafe accepts and task regressions, supported policy-eligible codecs,
    median and aggregate net benefit for every global/codec/stratum/cache/cell
    slice, and bounded aggregate and per-case latency. Evidence cannot weaken
    the runtime-owned limits. A valid failed gate emits no manifest.
...
| Tokenizer skew and fake savings              | Local counts use the wrong model tokenizer or omit decoder/legend/retry costs.                                                          | Record tokenizer ID/fingerprint; v0.1 local counts remain diagnostic; promotion accepts only exact provider/runtime observations and uses the lower of physical-input and normalized-cost savings.        | The host defines the normalization protocol and can attest dishonestly; its digest is integrity evidence, not independent metering.                                                 |
| Prompt-cache regression                      | Rewriting a stable prefix invalidates provider cache and costs more than it saves.                                                      | Fixed cold/warm coverage with five cases per difficulty/cache cell; median and weighted aggregate normalized-cost gates per cache regime and cell; paired order randomized or counterbalanced.            | Host/provider cache policies may be opaque, dynamic, or model-specific; normalized costs are host attestations scoped to the evaluated deployment.                                  |
| Replay poisoning or benchmark gaming         | A curated corpus omits failures, reuses traces, leaks expected answers, or overrepresents compressible logs.                            | Derived corpus digest; unique case/trace/quality digests; strict ordinal range; minimum 50 paired cases; fixed 4x2 coverage; failed and bypassed cases retained; positive bypass noise earns no savings.  | Distinct digests do not prove semantic independence, IID sampling, hidden holdout status, or oracle honesty; independent corpus governance remains necessary.                       |
...
- workbench validation proving strict payload-free JSONL, complete paired
  corpus accounting, replay resistance, deterministic `BigInt` median and
  aggregate metrics, order independence, codec/stratum/cache/cell and per-case
  gates, no-manifest failure semantics, symlink-safe private output creation,
  and interoperability with the active host preparer.
...
The bundled replay command checks mechanical expectations, not task quality. A
codec becomes eligible for the active host only when the Promotion Evidence
Workbench validates a complete held-out corpus and emits a strict manifest.
Admission requires zero unsafe accepts, zero measured task-quality regressions,
at least 10% median and aggregate net benefit globally and for every
codec/stratum/cache/cell slice after physical input and host-attested
normalized-cost overhead, and aggregate plus per-case latency within effective
runtime-owned ceilings.
That result remains scoped to the tested policy, tokenizer, host, adapter,
provider/model, prompt/tool contracts, selector/trust map, protocol, and corpus.
Because both evidence and manifest are unsigned host attestations, deployment
provenance remains a residual risk. A timed-out non-cooperative preparer may
continue local work after the original request has been forwarded;
worker/process isolation is required where hard cancellation is mandatory.
