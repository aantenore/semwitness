...
| False savings claim                                  | Raw candidate bytes and rendered bytes are counted locally; reliability is labelled; `billedOutputSavings: null`  | Only provider usage can establish real billing impact                                                                     |
...
| Witness correlation or dictionary inference          | Witnesses omit candidate and rendered payloads; keep artifacts local, private, access-controlled, and short-lived | Stable digests and lengths reveal equality and workload shape; low-entropy values may be recovered by dictionary guessing |
| Semantic error hidden by valid JSON                  | Schema proves structure only; evaluation and task-specific validation remain host duties                          | `universalSemanticEquivalence: false`; no universal semantic proof is claimed                                             |
...
Content-free does not mean confidential: do not publish witnesses or treat their
stable digests and lengths as anonymous telemetry. Keep them local and private
unless the equality, workload-shape, and low-entropy dictionary risks are
explicitly accepted.
